...
object FabEvents {
    fun init() {
        DefaultItemComponentEvents.MODIFY.register(::modifyItemComponents)
        injectSpawns()
        modifyEnchanting()
        registerComposting()
    }

    fun modifyItemComponents(ctx: DefaultItemComponentEvents.ModifyContext) {
        ctx.modify({ it is CustomSkillet }) { builder, item ->
